...
fn coswid_create(args: &CoswidCreateSubcommand) {
    ...
    for f in &files {
        coswid_template_to_cbor(f, output_dir);
    }
}
...
fn coswid_display(args: &DisplaySubcommand) {
    ...
    let cbor: ConciseSwidTagCbor = match from_reader(data.as_slice()) {
        Ok(c) => c,
        Err(e) => {
            println!(
    ...
    let json: ConciseSwidTag = match cbor.try_into() {
        Ok(s) => s,
        Err(_) => {
            println!(
...
fn coswid_template_to_cbor(template_file: &String, output_dir: &Path) {
    ...
    let comid_json: ConciseSwidTag = match serde_json::from_str(&data) {
        Ok(s) => s,
        Err(e) => {
            println!(
    ...
    let comid_cbor: ConciseSwidTagCbor = match comid_json.try_into() {
        Ok(s) => s,
        Err(_) => {
            println!(
