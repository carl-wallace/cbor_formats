...
- **Base type** (e.g., `CorimMetaMap`) — Uses standard serde `Serialize`/`Deserialize` derives. This works
  ...
  [ciborium](https://crates.io/crates/ciborium) for CBOR maps that use text-based keys. No additional
  code generation is needed because serde's default behavior already handles these formats.

- **`*Cbor` type** (e.g., `CorimMetaMapCbor`) — Generated by the proc macros in this crate. These types
  implement the CBOR-specific encoding rules that serde's default derives cannot express: maps keyed by
  integer labels (`StructToMap`) and positional arrays (`StructToArray`). This is the encoding style
  used by most CBOR-based IETF specifications (COSE, CoRIM, CoSWID, EAT, etc.).

`TryFrom` implementations are generated to convert between the two forms, allowing data to flow between
...
Each field annotated with `#[cbor(...)]` accepts three attributes:

| Attribute | Purpose | Required |
|-----------|---------|----------|
| `tag = "N"` | Integer key in the CBOR map (for `StructToMap`). Omit only for `other`-style `Vec<Tuple>` extension fields. Not used by `StructToArray` (position is implicit). | Yes (StructToMap) |
| `value = "T"` | How the field is represented as a ciborium `Value`. See table below. Omit when the field type has its own `TryFrom<&Value>` impl and serde traits. | No |
| `cbor = "true"` | The `*Cbor` struct should use a `Cbor`-suffixed type for this field (e.g., `Time` becomes `TimeCbor`). See guidance below. | No |

#### `value` types

| `value` | CBOR type | Rust type | Notes |
|---------|-----------|-----------|-------|
| `"Integer"` | unsigned/negative int | `u64`, `i64`, `i8`, etc. | |
| `"Text"` | text string | `String` | |
| `"Bytes"` | byte string | `Vec<u8>` | Required for `Vec<u8>` fields (otherwise ciborium serializes as an array of integers) |
| `"Bool"` | boolean | `bool` | |
| `"Map"` | map | struct with `StructToMap` | The inner type must implement `TryFrom<Vec<(Value, Value)>>` |
| `"Array"` | array | `Vec<T>` | Each element `T` must implement `TryFrom<Value>` |
| (omitted) | any | custom type | The type must implement `TryFrom<&Value>` for decoding and serde `Serialize` for encoding |

#### When to use `cbor = "true"`

The `cbor = "true"` attribute tells the macro that a field's type has a separate CBOR-specific
variant with a `Cbor` suffix (e.g., `TimeCbor`, `VerifierIdCbor`). This is needed when the JSON
and CBOR serializations of a type differ structurally — for example, when the CBOR form uses
integer map keys or CBOR tags that the JSON form does not.

**Use `cbor = "true"` when** the field type was itself derived with `StructToMap` or `StructToArray`,
or when the CDDL definition uses a CBOR tag that must be present on the wire (e.g., `time-int = #6.1(int)`
serializes with CBOR tag 1 via `TimeCbor`).

**Do not use `cbor = "true"` when** the CDDL uses the unwrap operator `~` to strip a tag. For example,
`~time-int` means the value is a plain integer without CBOR tag 1. In this case, use
`value = "Integer"` with a plain `i64` field instead of `cbor = "true"` with a `Time` field.

#### Common CDDL patterns and their attribute mappings

| CDDL | Rust field | Attributes | Notes |
|------|-----------|------------|-------|
| `label => tstr` | `field: String` | `tag = "N", value = "Text"` | |
| `label => int` | `field: i64` | `tag = "N", value = "Integer"` | |
| `label => bstr` | `field: Vec<u8>` | `tag = "N", value = "Bytes"` | |
| `label => bool` | `field: bool` | `tag = "N", value = "Bool"` | |
| `label => nested-map` (StructToMap) | `field: NestedMap` | `tag = "N", cbor = "true"` | `value = "Map"` is optional; without it, decoding uses `NestedMapCbor::try_from(&Value)`; with it, the macro extracts map entries first and calls `TryFrom<Vec<(Value, Value)>>` directly. Both are generated by StructToMap. |
| `label => nested-array` (StructToArray) | `field: NestedArray` | `tag = "N", cbor = "true"` | `NestedArrayCbor` must impl `TryFrom<&Value>` (generated by StructToArray) |
| `label => [ + nested-map ]` | `field: Vec<NestedMap>` | `tag = "N", value = "Array", cbor = "true"` | Each element decoded via `NestedMapCbor::try_from(Value)` |
| `label => [ + nested-array ]` | `field: Vec<NestedArray>` | `tag = "N", value = "Array", cbor = "true"` | Each element decoded via `NestedArrayCbor::try_from(Value)` |
| `label => time-int` (tagged) | `field: Time` | `tag = "N", cbor = "true"` | `TimeCbor` wraps with CBOR tag 1 |
